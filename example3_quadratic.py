# Code for Example 3 of our paper at https://arxiv.org/abs/2310.20310.
...
def E_analytical(v, t):
    ...
    return np.array([np.sin(np.pi*(np.sqrt(2)*t - (x+y))) - np.sin(np.pi*x)*np.cos(np.pi*t), 
                     -np.sin(np.pi*(np.sqrt(2)*t - (x+y))) - np.sin(np.pi*y)*np.cos(np.pi*t)])
...
def rot_W01_1(ell_Tet,dl_Tet):
    return rot_Wbasis(ell_Tet,dl_Tet,a=1,b=1,c=0,t=1) - rot_Wbasis(ell_Tet,dl_Tet,a=0,b=2,c=0,t=0)
def rot_W02_0(ell_Tet,dl_Tet):
    return rot_Wbasis(ell_Tet,dl_Tet,a=2,b=0,c=0,t=2) - rot_Wbasis(ell_Tet,dl_Tet,a=1,b=0,c=1,t=0)
def rot_W02_1(ell_Tet,dl_Tet):
    return rot_Wbasis(ell_Tet,dl_Tet,a=1,b=0,c=1,t=2) - rot_Wbasis(ell_Tet,dl_Tet,a=0,b=0,c=2,t=0)
def rot_W12_0(ell_Tet,dl_Tet):
...
def rot_W012_1(ell_Tet,dl_Tet):
    return rot_Wbasis(ell_Tet,dl_Tet,a=1,b=1,c=0,t=2) - rot_Wbasis(ell_Tet,dl_Tet,a=0,b=1,c=1,t=0)

# Discontinuous Galerkin (order = 1) basis on a physical triangle
def DG012_0(ell_T, vol_T):
    return 3/vol_T * ell_T[0]
def DG012_1(ell_T, vol_T):
    return 3/vol_T * ell_T[1]
def DG012_2(ell_T, vol_T):
    return 3/vol_T * ell_T[2]

# Lists referencing the various basis functions
V_Ws = [W0, W1, W2, W3, W4, W5]
grad_V_Ws = [grad_W0, grad_W1, grad_W2, grad_W3, grad_W4, grad_W5]
E_Ws = [W01_0, W01_1, W02_0, W02_1, W12_0, W12_1, W012_0, W012_1]
rot_E_Ws = [rot_W01_0, rot_W01_1, rot_W02_0, rot_W02_1, rot_W12_0, rot_W12_1, rot_W012_0, rot_W012_1]
T_DGs = [DG012_0, DG012_1, DG012_2]
    ...
    for i in range(len(grad_V_Ws)):    # Loop over vertices (p basis)
        for j in range(len(E_Ws)):    # Loop over edges (E basis)
            ...
            for kq, qp_b in enumerate(qnodes_bary):
                integral += np.dot(grad_V_Ws[i](qp_b, dl_T), 
...
def H_interpolation(H_T, ell_interp, vol_T):
    # Compute the interpolant at the interpolation point
    return np.sum([H_T[i] * T_DGs[i](ell_interp, vol_T) for i in range(len(T_DGs))], axis=0)
...
N0b = boundary_vertex_indices.shape[0]    # Number of vertices in the boundary
...
p_0 = np.zeros(N0+N1); E_0 = np.zeros(2*N1+2*N2); H_0 = np.zeros(3*N2)
...
for T_index, T in enumerate(tqdm.tqdm(sc[2].simplices)):
    ...
    for i in range(3):    # Loop over vertices
        for j in range(3):    # Loop over vertices/edges
            ...
            rows00.append(T[i])
            columns00.append(N0+edges_T[j])
            M00_data.append(M00[i, 3 + j])

            rows00.append(N0+edges_T[i])
            columns00.append(T[j])
            M00_data.append(M00[3 + i, j])

            rows00.append(N0+edges_T[i])
            columns00.append(N0+edges_T[j])
            M00_data.append(M00[3 + i, 3 + j])

            for k in range(2):    # Loop over edges
                # Integrate Local Stiffness Matrix 01 into Global Stiffness Matrix 01
                rows01.append(T[i])
                columns01.append(2*edges_T[j] + k)
                S01_data.append(S01[i, 2*j + k])

                rows01.append(N0+edges_T[i])
                columns01.append(2*edges_T[j] + k)
                S01_data.append(S01[3 + i, 2*j + k])

                # Integrate Local Mass Matrix 11 into Global Mass Matrix 11
                rows11.append(2*edges_T[i] + k)
                columns11.append(2*edges_T[j])
                M11_data.append(M11[2*i + k, 2*j])

                rows11.append(2*edges_T[i]+k)
                columns11.append(2*edges_T[j] + 1)
                M11_data.append(M11[2*i + k, 2*j + 1])

        for k in range(2):   # Loop over faces 
            # Integrate Local Stiffness Matrix 01 into Global Stiffness Matrix 01
            rows01.append(T[i])
            columns01.append(2*N1+2*T_index+k)
            S01_data.append(S01[i, 6 + k])

            rows01.append(N0+edges_T[i])
            columns01.append(2*N1+2*T_index+k)
            S01_data.append(S01[3 + i, 6 + k])

            # Integrate Local Mass Matrix 11 into Global Mass Matrix 11
            rows11.append(2*edges_T[i])
            columns11.append(2*N1 + 2*T_index + k)
            M11_data.append(M11[2*i, 6 + k])

            rows11.append(2*edges_T[i] + 1)
            columns11.append(2*N1 + 2*T_index + k)
            M11_data.append(M11[2*i + 1, 6 + k])

            rows11.append(2*N1 + 2*T_index + k)
            columns11.append(2*edges_T[i])
            M11_data.append(M11[6 + k, 2*i])            

            rows11.append(2*N1 + 2*T_index + k)
            columns11.append(2*edges_T[i] + 1)
            M11_data.append(M11[6 + k, 2*i + 1])

    for i in range(2):    # Loop over faces 
        for j in range(2):    # Loop over faces
            # Integrate Local Mass Matrix 11 into Global Mass Matrix 11     
            rows11.append(2*N1 + 2*T_index + i)
            columns11.append(2*N1 + 2*T_index + j)
            M11_data.append(M11[6 + i, 6 + j])
    ...
    for i in range(3):    # Loop over edges
        for j in range(3):    # Loop over faces
            for k in range(2):    # Loop over edges
                rows12.append(2*edges_T[i] + k)
                columns12.append(3*T_index + j)
                S12_data.append(S12[2*i + k, j])

    for i in range(2):    # Loop over faces
        for j in range(3):    # Loop over faces
            rows12.append(2*N1 + 2*T_index + i)
            columns12.append(3*T_index + j)
            S12_data.append(S12[6 + i, j])

    # Integrate Local Mass Matrix 22 into Global Mass Matrix 22
    # Note: This matrix is not part of the finite element system matrix
    for i in range(3):    # Loop over faces
        for j in range(3):    # Loop over faces
            rows22.append(3*T_index + i)
            columns22.append(3*T_index + j)
            M22_data.append(M22[i, j])

        
    # Computing Initial Vectors at t = T_min
    for i in range(3):
        for j in range(2):
            E_integral = 0
            for kq, qp_b in enumerate(qnodes_bary):
                ...
                E_integral += np.dot(E_analytical([x_q, y_q], T_min) , 
                                     E_Ws[2*i+j](qp_b, dl_T)) * qweights[kq]
            E_integral *= integral_scaling
            E_0[2*edges_T[i]+j] += E_integral
    ...
    for i in range(2):    
            ...
            E_integral *= integral_scaling
            E_0[2*N1 + 2*T_index + i] += E_integral
    ...
    for i in range(len(T_DGs)): 
            ...
            for kq, qp_b in enumerate(qnodes_bary):
                x_q = np.dot(qp_b, vertices_T[:, 0])
                y_q = np.dot(qp_b, vertices_T[:, 1])
                H_integral += np.dot(H_analytical([x_q, y_q], T_min), 
                                    T_DGs[i](qp_b, vol_phy_tri)) * qweights[kq]
            H_integral *= vol_phy_tri/vol_std_tri
            H_0[3*T_index + i] = H_integral
...
M11_g = M11_g.tocsr()
S12_g = sprs.coo_matrix((S12_data, (rows12, columns12)), (2*N1+2*N2, 3*N2), dtype='float')
S12_g = S12_g.tocsr()
M22_g = sprs.coo_matrix((M22_data, (rows22, columns22)), (3*N2, 3*N2), dtype='float')
M22_g = M22_g.tocsr()
...
for i, e in enumerate(sc[1].simplices):
    v0, v1 = sc.vertices[e]
    p_0[N0+i] = p_analytical((v0+v1)/2, t=0)
...
for i in range(2):
    M11_g_bndry[2*boundary_edge_indices+i] = 0
    M11_g_bndry[2*boundary_edge_indices+i, 2*boundary_edge_indices+i] = 1
    E_0[2*boundary_left_edges+i] = E0_left_boundary
    E_0[2*boundary_right_edges+i] = E0_right_boundary
    E_0[2*boundary_top_edges+i] = E0_top_boundary
    E_0[2*boundary_bottom_edges+i] = E0_bottom_boundary
...
if use_backward_euler:  
    ...
    for time_step in tqdm.tqdm(range(1, number_of_time_steps)):
        b_p = np.zeros(N0+N1); b_E = np.zeros(2*N1+2*N2); b_H = np.zeros(3*N2); H_h = np.zeros(3*N2)
        for T_index, T in enumerate(sc[2].simplices):
            ...
            for kq, qp_b in enumerate(qnodes_bary):
                ...
                for i in range(3):
                    fp_integral = 0
                    fp_integral += np.dot(fp_analytical([x_q, y_q], (time_step) * dt), 
                ...
                for i in range(3):
                    fp_integral = 0
                    fp_integral += np.dot(fp_analytical([x_q, y_q], (time_step) * dt), 
                                               V_Ws[3+i](qp_b)) * qweights[kq]
                    fp_integral *= integral_scaling
                    b_p[N0 + edges_T[i]] += fp_integral

                for i in range(3):
                    for j in range(2):
                        fE_integral = 0
                        fE_integral += np.dot(fE_analytical([x_q, y_q], (time_step) * dt), 
                                                     E_Ws[2*i+j](qp_b, dl_T)) * qweights[kq]
                        fE_integral *= integral_scaling 
                        b_E[2*edges_T[i]+j] += fE_integral      
                for i in range(2):
                    fE_integral = 0
                    fE_integral += np.dot(fE_analytical([x_q, y_q], (time_step) * dt), 
                                                     E_Ws[2*3+i](qp_b, dl_T)) * qweights[kq]
                    fE_integral *= integral_scaling
                    b_E[2*N1+2*T_index+i] += fE_integral

                for i in range(3):
                    fH_integral = 0
                    H_integral = 0
                    fH_integral += np.dot(fH_analytical([x_q, y_q], (time_step) * dt), 
                                             T_DGs[i](qp_b, vol_phy_tri)) * qweights[kq]
                    H_integral += np.dot(H_analytical([x_q, y_q], (time_step) * dt), 
                                            T_DGs[i](qp_b, vol_phy_tri)) * qweights[kq]
                    fH_integral *= integral_scaling
                    H_integral *= integral_scaling
                    b_H[3*T_index+i] += fH_integral
        ...
        bp_RHS[boundary_vertex_indices] = p_bc[:N0b]
        bp_RHS[N0 + boundary_edge_indices] = p_bc[N0b:]
        for i in range(2):
            bE_RHS[2*boundary_left_edges+i] = E_left_boundary
            bE_RHS[2*boundary_right_edges+i] = E_right_boundary
            bE_RHS[2*boundary_top_edges+i] = E_top_boundary
            bE_RHS[2*boundary_bottom_edges+i] = E_bottom_boundary
...
if use_leap_frog:  
    ...
    b0_p = np.zeros(N0+N1); b0_E = np.zeros(2*N1+2*N2); b0_H = np.zeros(3*N2)
    for T_index, T in enumerate(sc[2].simplices):
        vertices_T = sc.vertices[T]
        vol_phy_tri = sc[2].primal_volume[T_index]
        dl_T = dl(vertices_T)
        ...
        for kq, qp_b in enumerate(qnodes_bary):
            ...
            for i in range(3):
                fp_integral = 0
                fp_integral += np.dot(fp_analytical([x_q, y_q], 0), 
                                            V_Ws[i](qp_b)) * qweights[kq]
                fp_integral *= integral_scaling
                b0_p[T[i]] += fp_integral
            for i in range(3):
                fp_integral = 0
                fp_integral += np.dot(fp_analytical([x_q, y_q], 0), 
                                            V_Ws[3+i](qp_b)) * qweights[kq]
                fp_integral *= integral_scaling
                b0_p[N0 + edges_T[i]] += fp_integral

            for i in range(3):
                for j in range(2):
                    fE_integral = 0
                    fE_integral += np.dot(fE_analytical([x_q, y_q], 0), 
                                                    E_Ws[2*i+j](qp_b, dl_T)) * qweights[kq]
                    fE_integral *= integral_scaling 
                    b0_E[2*edges_T[i]+j] += fE_integral      
            for i in range(2):
                fE_integral = 0
                fE_integral += np.dot(fE_analytical([x_q, y_q], 0), 
                                                    E_Ws[2*3+i](qp_b, dl_T)) * qweights[kq]
                fE_integral *= integral_scaling
                b0_E[2*N1+2*T_index+i] += fE_integral

            for i in range(3):
                fH_integral = 0
                H_integral = 0
                fH_integral += np.dot(fH_analytical([x_q, y_q], 1/2*dt), 
                                            T_DGs[i](qp_b, vol_phy_tri)) * qweights[kq]
                H_integral += np.dot(H_analytical([x_q, y_q], 1/2*dt), 
                                        T_DGs[i](qp_b, vol_phy_tri)) * qweights[kq]
                fH_integral *= integral_scaling
                H_integral *= integral_scaling
                b0_H[3*T_index+i] += fH_integral
    ...
    p0_bc = np.array([p_analytical(v, 0) for v in p_boundary_vertices])
    ...
    for time_step in tqdm.tqdm(range(2, number_of_time_steps)):
        ...
        for T_index, T in enumerate(sc[2].simplices):
            ...
            for kq, qp_b in enumerate(qnodes_bary):
                ...
                for i in range(3):
                    fp_integral = 0
                    fp_integral += np.dot(fp_analytical([x_q, y_q], (time_step-1)*dt), 
                                          V_Ws[i](qp_b)) * qweights[kq]
                    fp_integral *= integral_scaling
                    b_p[T[i]] += fp_integral
                for i in range(3):
                ...
                for i in range(3):
                    for j in range(2):
                        fE_integral = 0
                        fE_integral += np.dot(fE_analytical([x_q, y_q], (time_step-1)*dt), 
                                              E_Ws[2*i+j](qp_b, dl_T)) * qweights[kq]
                        fE_integral *= integral_scaling 
                        b_E[2*edges_T[i]+j] += fE_integral      
                for i in range(2):
                ...
                for i in range(3):
                    fH_integral = 0
                    H_integral = 0
                    fH_integral += np.dot(fH_analytical([x_q, y_q], (time_step-1/2)*dt), 
                                          T_DGs[i](qp_b, vol_phy_tri)) * qweights[kq]
                    H_integral += np.dot(H_analytical([x_q, y_q], (time_step-1/2)*dt), 
                                         T_DGs[i](qp_b, vol_phy_tri)) * qweights[kq]
                    fH_integral *= integral_scaling
                    H_integral *= integral_scaling
                    b_H[3*T_index+i] += fH_integral
        ...
        p_bc = np.array([p_analytical(v, (time_step-1) * dt) for v in p_boundary_vertices])
...
if plot_solutions == True:
    ...
    if use_leap_frog == True:
        for pts_index, plot_time_step in enumerate(plot_time_steps):
            ...
            for T_index, T in enumerate(tqdm.tqdm(sc[2].simplices)):
                ...
                for i in range(3):    # Loop over edges
                    for j in range(2):    # Loop over edges
                        E_T.append(E_plot_time[2*edges_T[i] + j])
                
                for i in range(2):    # Loop over faces
                    E_T.append(E_plot_time[2*N1 + 2*T_index + i])
                E_T = np.array(E_T)
                ...
                H_T = []
                for i in range(3):
                    H_T.append(H_plot_time[3*T_index + i])
                H_T = np.array(H_T) 
            ...
            tpc = plt.tripcolor(Vertices[:, 0], Vertices[:, 1], p_plot_time[:N0], 
...
if compute_energy == True:
    ...
    for pts_index, solution_time_step in enumerate(tqdm.tqdm(solution_time_steps)):
        ...
        for T_index, T in enumerate(sc[2].simplices):
            ...
            p_T = []
            for i in range(3):
                p_T.append(p_solution_time[T[i]])
            for i in range(3):
                p_T.append(p_solution_time[N0 + edges_T[i]])
            ...
            for i in range(3):    # Loop over edges
                for j in range(2):    # Loop over edges
                    E_T.append(E_solution_time[2*edges_T[i] + j])
            
            for i in range(2):    # Loop over faces
                E_T.append(E_solution_time[2*N1 + 2*T_index + i])
            E_T = np.array(E_T)

            # Obtain the restriction of discrete H to this triangle and interpolate it
            H_T = []
            for i in range(3):
                H_T.append(H_solution_time[3*T_index + i])
            H_T = np.array(H_T) 
